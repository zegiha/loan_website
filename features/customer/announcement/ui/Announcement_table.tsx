...
export default function Announcement_table() {
  ...
  const { data, status, error } = useNoticeControllerGetNotices({
    query: {
      select: (v) => {
...
function Announcement_table_row({
  ...
  return (
    <Link href={`/customer/announcement/${post_id}`} style={{ width: "100%" }}>
      <TableRow
        className={type === "variable" ? style.notificationRow : undefined}
      >
        <Typo.Contents
          width={30}
          color={type === "variable" ? "primary" : "generic"}
        >
          {type === "variable" ? "중요" : "일반"}
        </Typo.Contents>
        <Typo.Contents width={"fill"} textOverflowLine={2} isPre>
          {title}
        </Typo.Contents>
        {show_sub_status && (
          <>
            <Typo.Contents
              ...
              color={"dim"}
              isPre={"nowrap"}
            >{`${created_date.getFullYear()}.${created_date.getMonth() + 1}.${created_date.getDate()}`}</Typo.Contents>
          </>
        )}
      </TableRow>
    </Link>
